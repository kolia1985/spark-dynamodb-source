...
case class DynamoDBSourceOffset(shardsToOffsets: ShardOffsets) extends OffsetV2 {
  override def json: String = {
    ...
    Serialization.write(result)(DynamoDBSourceOffset.format)
    }
}

private[dynamodb]
case class DynamoDBSourcePartitionOffset(shardId: String, shardInfo: ShardInfo)
  extends PartitionOffset


object DynamoDBSourceOffset {
  implicit val format = Serialization.formats(NoTypeHints)

  def getShardOffsets(offset: Offset): ShardOffsets = {
    offset match {
      case kso: DynamoDBSourceOffset => kso.shardsToOffsets
      case so: SerializedOffset => DynamoDBSourceOffset(so).shardsToOffsets
      case _ => throw
        new IllegalArgumentException(s"Invalid conversion " +
          s"from offset of ${offset.getClass} to DynamoDBSourceOffset")
    }
  }

  /*
   * Returns [[DynamoDBSourceOffset]] from a JSON [[SerializedOffset]]
   */
  def apply(so: SerializedOffset): DynamoDBSourceOffset = {
    apply(so.json)
  }

  /*
   * Returns [[DynamoDBSourceOffset]] from a JSON
   */
  def apply(json: String): DynamoDBSourceOffset = {
    try {
      ...
      DynamoDBSourceOffset(new ShardOffsets(metadata.get("batchId").toLong,
