...
/** Offset range that one partition of the DynamoDBSourceRDD has to read */
private[dynamodb] case class ShardInfo(
                                        shardId: String,
                                        iteratorType: String,
                                        iteratorPosition: String) extends Serializable {

  def this(shardId: String, dynamodbPosition: DynamoDBPosition) {
    this(shardId, dynamodbPosition.iteratorType, dynamodbPosition.iteratorPosition)
  }
}

private[dynamodb] case class ShardOffsets(
                                           batchId: Long,
                                           streamName: String,
...
/** Partition of the DynamoDBSourceRDD */
private[dynamodb] case class DynamoDBSourceRDDPartition(
                                                        index: Int,
                                                        shardInfo: ShardInfo) extends Partition

/*
 * An RDD that reads data from DynamoDB based on offset ranges across multiple shards.
 */
private[dynamodb] class DynamoDBSourceRDD(
  ...
  override def persist(newLevel: StorageLevel): this.type = {
    logError("DynamoDB Record is not serializable. " +
      "Use .map to extract fields before calling .persist or .window")
    super.persist(newLevel)
  }

  override def getPartitions: Array[Partition] = {
    shardInfos.zipWithIndex.map { case (o, i) => new DynamoDBSourceRDDPartition(i, o) }.toArray
  }

  override def compute(
      thePart: Partition,
      context: TaskContext): Iterator[Record] = {
    val sourcePartition = thePart.asInstanceOf[DynamoDBSourceRDDPartition]

    val dynamodbShardId = sourcePartition.shardInfo.shardId

    val dynamodbReader = new DynamoDBReader(
      sourceOptions,
      streamName,
      credentialsProvider,
      endpointUrl,
    ...
    val underlying = new NextIterator[Record]() {
      ...
      def getShardIterator(): String = {
        if (_shardIterator == null) {
          _shardIterator = dynamodbReader.getShardIterator(
        ...
        if (fetchedRecords.length == 0 || currentIndex >= fetchedRecords.length) {
          ...
          while (fetchedRecords.length == 0 && fetchNext == true)  {
            ...
            if (currentTimestamp - startTimestamp < maxFetchTimeInMs) {
              ...
              val records: GetRecordsResult = dynamodbReader.getDynamoDBRecords(getShardIterator,
                recordPerRequest)
              // de-aggregate records
               val deaggregateRecords = dynamodbReader.deaggregateRecords(records.getRecords, null)
              ...
              val millisBehindLatest = 0//records.getMillisBehindLatest.longValue()
              logDebug(s"Milli secs behind is ${millisBehindLatest}")
              lastSeenTimeStamp = currentTimestamp - millisBehindLatest
              if ( _shardIterator == null ) {
                hasShardClosed = true
                fetchNext = false
              }
      ...
      metaDataCommitterType.toLowerCase(Locale.ROOT) match {
        ...
        case "dbfs" => {
          if (metaDataCommitterPath.startsWith("dbfs")) {
            ...
            new HDFSMetadataCommitter[ ShardInfo ](metaDatapath, conf)
            } else {
    ...
    def updateMetadata(taskContext: TaskContext): Unit = {
      ...
      logInfo(s"Batch $batchId : Committing End Shard position for $dynamodbShardId")
      metadataCommitter.add(batchId, dynamodbShardId, shardInfo)
    }
