...
/*
 * Helper class to sync batch with shards of the DynamoDB stream.
 * It will create new activities when it discovers new DynamoDB shards (bootstrap/resharding).
 * It works in similar way as
 * com.amazonaws.services.kinesis.clientlibrary.lib.worker.ShardSyncer in KCL
 */

private[dynamodb] object ShardSyncer extends Logging {
  ...
  private[dynamodb] def AddShardInfoForAncestors(
                                                  shardId: String,
  ...
  private[dynamodb] def getParentShardIds(
     shard: Shard,
  ...
  def getLatestShardInfo(
      ...
      initialPosition: DynamoDBPosition): Seq[ShardInfo] = {
